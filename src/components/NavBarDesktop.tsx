...
const NavBarDesktop = () => {
  ...
  return (
    <section className="hidden lg:block relative z-50">
      <nav className="flex py-8  items-center justify-between max-w-6xl mx-auto">
        <svg
        ...
        <ul className="flex relative text-white items-center">
          <li className="relative">
            <button
              className="mr-2 px-6 py-3 font-medium flex items-center gap-x-2 cursor-pointer"
              onClick={() => setShowSubMenu(!showSubMenu)}
            >
              Solutions <IoIosArrowDown />
            </button>
            <ul
              className={` absolute bg-white text-slate-500  rounded *:border-b *:border-blue-400 last:*:border-none w-56 *:hover:text-blue-500 ${
                showSubMenu ? "block" : "hidden"
              }`}
            >
              <li>
                <Link className="px-4 py-3 inline-block" href={"/"}>
                  AnyCaaS
                </Link>
              </li>
              <li>
          ...
          <li>
            <button
              className="text-white text-base border px-4  rounded-full py-3 flex items-center gap-x-2 cursor-pointer"
              onClick={() => setShowLanguages(!showLanguages)}
            >
              <RiGlobalFill /> EN <IoIosArrowDown />
            </button>
            <ul
              className={`absolute overflow-visible bottom-0 translate-y-full bg-white rounded text-black  p-3 divide-y w-40 ${
                showLanguages ? "block" : "hidden"
              }`}
            >
              <li className="py-1 hover:text-blue-main hover:transition-colors duration-200 text-slate-700">
                EN (English)
              </li>
              <li className="py-1 hover:text-blue-main hover:transition-colors duration-200 text-slate-700">
                TH (THai)
        ...
        <button className="text-white text-xl border pl-8 pr-6 rounded-sm py-3 flex items-center gap-x-2 tracking-wide font-medium">
