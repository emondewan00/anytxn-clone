...
const MobileNav = () => {
  ...
  return (
    <nav className="relative z-50 block lg:hidden">
      <div className="flex items-center justify-between bg-[#1F80F0] py-6 px-4">
        <svg
        ...
        >
          {openMenu ? <IoCloseSharp /> : <RxHamburgerMenu />}
        </button>
      </div>

      <div
        className={`absolute  w-full p-8 bg-[#1B76E9] transition-all duration-150 delay-75 -translate-x-full  ${
      ...
      >
        <ul>
          <li>
            <button
              className="px-6 py-3 text-white font-medium flex justify-between items-center w-full cursor-pointer"
              onClick={() => setSubMenuShow(!subMenuShow)}
            >
              Solutions <IoIosArrowDown />
            </button>
            <ul
              className={`overflow-hidden text-white ml-6 transition-all duration-300 ease-in-out ${
                subMenuShow ? "max-h-[300px] block" : "max-h-0 hidden"
              }`}
            >
              <li>
        ...
        <div className="flex items-center justify-center mt-6 mb-12 ">
          <div className="border border-gray-50/20 rounded-xl px-4 py-2 ">
            <button
              className="flex items-center gap-2 text-white justify-between w-full cursor-pointer "
            ...
            <ul
              className={`overflow-hidden text-white mt-2 ml-[23px] transition-[max-height] duration-300 ease-in-out ${
                showLanguages ? "max-h-[150px]" : "max-h-0"
              }`}
            >
              <li className="py-1 hover:text-blue-main hover:transition-colors duration-200">
                English
              </li>
              <li className="py-1 hover:text-blue-main hover:transition-colors duration-200">
                Spanish
